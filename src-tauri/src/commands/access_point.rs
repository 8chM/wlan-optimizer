...
pub fn update_access_point(
    ...
    conn.execute(
        "UPDATE access_points SET
            ...
            channel_24ghz = COALESCE(?7, channel_24ghz),
            channel_5ghz = COALESCE(?8, channel_5ghz),
            channel_width = COALESCE(?9, channel_width),
            enabled = COALESCE(?10, enabled),
            updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE id = ?11",
        rusqlite::params![
            params.x, params.y, params.height_m, params.mounting,
            params.tx_power_24ghz_dbm, params.tx_power_5ghz_dbm,
            params.channel_24ghz, params.channel_5ghz,
