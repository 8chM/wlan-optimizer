...
describe('RF model constants', () => {
  ...
  it('MIN_DISTANCE is positive and prevents log(0) singularity', () => {
    ...
    expect(MIN_DISTANCE).toBeLessThanOrEqual(0.5);
  });
});
...
describe('createRFConfig', () => {
  it('creates default config for 2.4 GHz with correct reference loss', () => {
    const config = createRFConfig('2.4ghz');
    expect(config.referenceLoss).toBeCloseTo(40.05, 2);
    expect(config.pathLossExponent).toBe(DEFAULT_PATH_LOSS_EXPONENT);
...
describe('computePathLoss', () => {
  ...
  describe('known distances', () => {
    it('at 10m for 2.4 GHz: PL = 40.05 + 35 = 75.05 dB', () => {
      // log10(10) = 1, so 10 * 3.5 * 1 = 35
      const pl = computePathLoss(10, config24);
      expect(pl).toBeCloseTo(75.05, 2);
    });

    it('at 10m for 5 GHz: PL = 46.42 + 35 = 81.42 dB', () => {
      const pl = computePathLoss(10, config5);
      expect(pl).toBeCloseTo(81.42, 2);
    });

    it('at 100m for 2.4 GHz: PL = 40.05 + 70 = 110.05 dB', () => {
      // log10(100) = 2, so 10 * 3.5 * 2 = 70
      const pl = computePathLoss(100, config24);
      expect(pl).toBeCloseTo(110.05, 2);
    });

    it('at 5m for 2.4 GHz: PL = 40.05 + 10*3.5*log10(5)', () => {
      const expected = 40.05 + 10 * 3.5 * Math.log10(5);
  ...
  describe('path loss exponent influence', () => {
    ...
    it('calibrated n=2.8 differs from default n=3.5 by 7 dB at 10m', () => {
      // Difference = 10*(3.5 - 2.8)*log10(10) = 10*0.7*1 = 7.0 dB
      ...
      expect(plDefault - plCalibrated).toBeCloseTo(7.0, 2);
    });
  });

  describe('band comparison', () => {
    it('5 GHz has ~6.37 dB more path loss than 2.4 GHz at the same distance', () => {
      // Reference loss difference: 46.42 - 40.05 = 6.37 dB
      const diff = computePathLoss(10, config5) - computePathLoss(10, config24);
      expect(diff).toBeCloseTo(6.37, 1);
    });

    it('6 GHz has ~1.54 dB more path loss than 5 GHz at the same distance', () => {
      // 47.96 - 46.42 = 1.54 dB
      const diff = computePathLoss(10, config6) - computePathLoss(10, config5);
      expect(diff).toBeCloseTo(1.54, 1);
    });

    it('band difference is constant regardless of distance (only PL(1m) differs)', () => {
      ...
      expect(diff10).toBeCloseTo(diff50, 4);
    });
  });
});
...
describe('computeRSSI', () => {
  ...
  describe('DAP-X2810 reference values at 2.4 GHz', () => {
    const config = createRFConfig('2.4ghz');
    ...
    it('at 10m (AP at origin, receiver at x=10): RSSI = 23 + 3.2 - 3 - 75.05 = -51.85 dBm', () => {
      const ap = makeAP(0, 0, 23, 3.2);
  ...
  describe('DAP-X2810 reference values at 5 GHz', () => {
    const config = createRFConfig('5ghz');
    ...
    it('at 10m: RSSI = 26 + 4.3 - 3 - 81.42 = -54.12 dBm', () => {
      const ap = makeAP(0, 0, 26, 4.3);
      const rssi = callComputeRSSI(10, 0, ap, config);
  ...
  describe('wall loss integration (mocked)', () => {
    ...
    // RSSI_base = 23 + 3.2 - 3 - 75.05 = -51.85 dBm
    const BASE_RSSI = 23 + 3.2 + (-3) - 75.05;
  ...
  describe('distance calculation', () => {
    ...
    it('RSSI at AP position matches expected MIN_DISTANCE calculation', () => {
      ...
      const expectedPL = 40.05 + 10 * 3.5 * Math.log10(MIN_DISTANCE);
